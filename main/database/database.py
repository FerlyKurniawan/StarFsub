...
class DatabaseClient:
    ...
    def add_ubot(self, user_id, api_id, api_hash, bot_token):
        self._check_connection()
        with self._connection as conn:
            cursor = conn.cursor()
            cursor.execute(
                """
                INSERT OR REPLACE INTO ubotdb (user_id, api_id, api_hash, bot_token)
                VALUES (?, ?, ?, ?)
                """,
                (user_id, api_id, api_hash, bot_token),
            )
    ...
    def get_userbots(self):
        ...
        with self._connection as conn:
            ...
            data = [
                ...
                for user_id, api_id, api_hash, bot_token in cursor.fetchall()
            ]
